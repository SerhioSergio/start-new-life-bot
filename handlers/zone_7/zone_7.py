...
async def start_day_for_zone7(message: Message, context: dict):
    ...
    await message.answer(
        ...
        "День 1 — Утро.\n"
        "Сегодня ты начнёшь возвращать себе силы и ясность.\n\n"
        "Напиши «Продолжить», когда будешь готов к перезапуску."
    )

async def send_morning_prompt(context, user_id, day):
    text = (
        f"*{ZONE_NAME}* — День {day} Утро\n\n"
        "Как ты понимаешь, что выгорел?\n"
        "Что в тебе просит паузы, отдыха, восстановления?"
    )
...
async def send_evening_prompt(context, user_id, day):
    text = (
        ...
        "Удалось ли тебе сегодня немного наполниться?\n"
        "Что тебе особенно помогло?"
    )
    await context.bot.send_message(chat_id=user_id, text=text, parse_mode="Markdown")

async def analyze_user_reply(message: Message):
    user_text = message.text
    feedback = await analyze_and_reply(user_text, zone="zone_7")
