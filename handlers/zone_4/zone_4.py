...
ZONE_NAME = "Зона 4 — Прокрастинация и личная дисциплина"
STEPS = [
...
async def start_day_for_zone4(message: Message, context: dict):
    ...
    await message.answer(
        ...
        "День 1 — Утро.\n"
        "Сегодня ты начнёшь ломать петлю откладывания.\n\n"
...
async def send_morning_prompt(context, user_id, day):
    text = (
        ...
        "Что внутри тебя не даёт начать?"
    )
...
async def send_evening_prompt(context, user_id, day):
    text = (
        ...
        "Удалось ли тебе сдвинуться с места?\n"
...
async def analyze_user_reply(message: Message):
    ...
    feedback = await analyze_and_reply(user_text, zone="zone_4")
