...
async def process_zone_4(message: Message):
    user_id = message.from_user.id
    step = get_user_step(user_id, "zone_4") or 0
    ...
    if message.text.lower() in ["продолжить", "назад", "выйти"] and step > 0:
        if message.text.lower() == "продолжить" and step < len(steps) - 1:
            ...
            return
        save_user_step(user_id, "zone_4", step)

    await message.answer(steps[step], reply_markup=get_step_keyboard())
    save_user_step(user_id, "zone_4", step)

def register_zone_4_handlers(dp: Dispatcher):
    dp.register_message_handler(process_zone_4, lambda msg: msg.text.lower() in ["продолжить", "назад", "выйти"])
async def process_zone_4_process(message: Message):
