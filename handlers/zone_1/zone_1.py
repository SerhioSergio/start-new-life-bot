...
async def start_day_for_zone1(message: Message, context):
    user_id = message.from_user.id
    await save_user_step(user_id, "zone_1", "День_1_утро")

    await message.answer(f"Ты начал работу с {ZONE_NAME}")
    await message.answer("День 1. Утро. Готов прокачивать своё мышление и уверенность?")

async def send_morning_prompt(context, user_id, day):
    await context.bot.send_message(user_id, f"{ZONE_NAME} — День {day} утро. Сегодня мы начнём с фундамента. Что для тебя деньги?")

async def send_evening_prompt(context, user_id, day):
    await context.bot.send_message(user_id, f"{ZONE_NAME} — День {day} вечер. Подведи итоги. Что ты понял о своей самооценке?")
