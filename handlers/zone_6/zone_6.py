...
ZONE_NAME = "Зона 6 — Выход из одиночества и связь с людьми"
STEPS = [
...
async def start_day_for_zone6(message: Message, context: dict):
    ...
    await message.answer(
        ...
        "День 1 — Утро.\n"
        "Сегодня мы начнём разрушать стену одиночества внутри.\n\n"
        "Напиши «Продолжить», когда будешь готов к сближению."
    )
...
async def send_morning_prompt(context, user_id, day):
    text = (
        ...
        "Чего ты на самом деле хочешь в этот момент?"
    )
...
async def send_evening_prompt(context, user_id, day):
    text = (
        ...
        "Удалось ли тебе сегодня почувствовать близость?\n"
        "Что ты сделал, чтобы приблизиться к другому?"
    )
    await context.bot.send_message(chat_id=user_id, text=text, parse_mode="Markdown")

async def analyze_user_reply(message: Message):
    user_text = message.text
    feedback = await analyze_and_reply(user_text, zone="zone_6")
